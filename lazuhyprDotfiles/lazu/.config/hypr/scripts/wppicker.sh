...
# Generate thumbnails
find "$WALLPAPER_DIR" -type f \( -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.png' -o -iname '*.webp' \) | while read -r imagen; do
    filename="$(basename "$imagen")"
    hash="$(sha1sum "$imagen" | awk '{print $1}')"
    thumb="$THUMB_DIR/$hash.sqre"
    if [ ! -f "$thumb" ]; then
        magick convert -strip "$imagen" -thumbnail x540^ -gravity center -extent 262x540 "$thumb" 2>/dev/null
    fi
done
